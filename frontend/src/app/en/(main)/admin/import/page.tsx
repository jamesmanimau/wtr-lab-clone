...
export default function AdminImportPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-3 mb-6">
        <div className="flex-1 relative">
          <input
            ...
            className="w-full bg-[#1e1e3a] border border-[#2a2a4a] rounded-xl pl-4 pr-12 py-3 text-sm text-gray-200 outline-none focus:border-[#2193b0]"
          />
          <button
            onClick={handleSearch}
            disabled={searching}
            className="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-[#2193b0] hover:bg-[#1a7a94] disabled:opacity-50 text-white text-sm rounded-lg transition-colors"
          >
      ...
      {/* Results */}
      {searching && <p className="text-sm text-[#2193b0] mb-4">Searching...</p>}
      ...
      {results.length > 0 && (
        <div className="space-y-3">
          {results.map((r) => (
              <div
                key={r.id}
                className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 p-4 bg-[#12122a] border border-[#1e1e3a] rounded-xl"
              >
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  {r.image ? (
                    <img src={r.image} alt="" className="w-10 h-14 sm:w-12 sm:h-16 rounded-lg object-cover bg-[#1e1e3a] shrink-0" />
                  ) : (
                    <div className="w-10 h-14 sm:w-12 sm:h-16 rounded-lg bg-[#1e1e3a] flex items-center justify-center shrink-0">
                ...
                <button
                  ...
                  className="w-full sm:w-auto shrink-0 px-4 py-2 bg-[#2193b0] hover:bg-[#1a7a94] disabled:opacity-50 text-white text-sm rounded-lg transition-colors"
                >
